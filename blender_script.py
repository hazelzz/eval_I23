...
parser.add_argument("--camera_dist", type=int, default=2)
...
def load_object(object_path: str) -> None:
    ...
    if object_path.endswith(".glb"):
        ...
        # bpy.ops.wm.obj_import(filepath=object_path, filter_glob = os.path.join(os.path.split(object_path)[0], "model.mtl")) # LEGO
        bpy.ops.wm.obj_import(filepath=object_path, forward_axis = 'X_FORWARD', up_axis= 'Z_UP' , filter_glob = os.path.join(os.path.split(object_path)[0], "model.mtl")) # LEGO
    else:
...
def save_images(object_file: str) -> None:
    ...
    for i in range(args.num_images):
        ...
        # Enable nodes and get reference to node tree
        ...
        # Create Map Value node to map depth to [0, 1] range
        normal_value_node = tree.nodes.new('CompositorNodeMapValue')
        # Set offset and size to control the depth range
        ...
        links.new(render_layers_node.outputs['Depth'], normal_value_node.inputs[0])
        links.new(normal_value_node.outputs[0],output_node.inputs[0])

        render_path = os.path.join(args.output_dir, object_uid, f"{i:03d}-depth.png")
        scene.render.filepath = os.path.abspath(render_path)
        bpy.ops.render.render(write_still=True)
        tree.links.clear()
